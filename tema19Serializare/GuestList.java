...
public class GuestList {
	...
	public GuestList(int maxGuestsNumber) throws IOException {
		if (Files.exists(Paths.get("TemeDevmind/tema19Serializare/guests.dat"))) {
			this.guestList = Guest.readFromBinaryFile();
		} else {
	...
	// Metoda auxiliara care ne spune daca o persoana se afla sa nu in lista
	// evenimentului
	private boolean isConfirmedGuest(Guest guest) {
		return searchGuestByName(guest.getLastName(), guest.getFirstName()) >= 0 || searchGuestByEmail(guest.getEmail()) >= 0
				|| searchGuestByPhoneNumber(guest.getPhoneNumber()) >= 0;
	}
	...
	// Metoda de cautare a persoanelor dupa un sir de caractere introdus
